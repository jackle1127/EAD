...
function feedData(dataString){
    ...
    var div = document.getElementById('result');
	var logResultsContainer = document.createElement('div');
	logResultsContainer.id = "logResults";
	...
	logResultsContainer.appendChild(tbl);
    div.appendChild(logResultsContainer);
	var downloadLogButton = document.createElement('button');
	var downloadLogTitle = document.createTextNode("Download Log");
	downloadLogButton.appendChild(downloadLogTitle);
	downloadLogButton.id = "btnDownloadLog";
	downloadLogButton.onclick = function() { downloadLog() };
	div.appendChild(downloadLogButton);
	...
	averagesTsv += "\tApproval";
	td.appendChild(document.createElement('br'));
	td.appendChild(document.createTextNode("- - - - - - -"))
    tr.appendChild(td);
    tbdy.appendChild(tr);
	(numCol == 2) ? averagesTsv += "\n- - - - - - -\t- - - - - - -\t- - - - - - -" : averagesTsv += "\n- - - - - - -\t- - - - - - -";
    for(var year in result[1]){
        ...
        if(this.aggregationPeriod == "yearly"){
            ...
            tr.appendChild(td);
			averagesTsv += year.substring(0, 4);
            var td = document.createElement('td');
            td.appendChild(document.createTextNode(year.substring(4, year.length)));
            tr.appendChild(td);
			averagesTsv += "\t" + year.substring(4, year.length);
        }
        td = document.createElement('td');
        td.appendChild(document.createTextNode(result[1][year].toFixed(3)));
        tr.appendChild(td);
        tbdy.appendChild(tr);
		averagesTsv += "\t" + result[1][year].toFixed(3);
    }
    tbl.appendChild(tbdy);
	moodAveragesContainer.appendChild(tbl);
    div.appendChild(moodAveragesContainer);
	var downloadAveragesButton = document.createElement('button');
	var downloadAveragesTitle = document.createTextNode("Download Averages");
	downloadAveragesButton.appendChild(downloadAveragesTitle);
	downloadAveragesButton.id = "btnDownloadLog";
	downloadAveragesButton.onclick = function() { downloadAverages() };
	div.appendChild(downloadAveragesButton);
}
