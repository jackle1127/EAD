...
// Function to hide download links when users make changes to the chart.
...
function downloadLog() {
	var saveLog = document.createElement('a');
	var mimeType = mimeType || 'text/plain';
	saveLog.download = "logResults.tsv";
	saveLog.setAttribute('href', 'data:' + mimeType  +  ';charset=utf-8,' + encodeURIComponent(logTsv));
	saveLog.click();
}

function downloadAverages() {
	var saveAverages = document.createElement('a');
	var mimeType = mimeType || 'text/plain';
	saveAverages.download = "averageApproval.tsv";
	saveAverages.setAttribute('href', 'data:' + mimeType + ';charset=utf-8,' + encodeURIComponent(averagesTsv));
	saveAverages.click();
}
